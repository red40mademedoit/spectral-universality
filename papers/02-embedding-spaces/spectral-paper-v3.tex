...
and q-bio.MN ($z = 1.4$, $p = 0.08$).  These categories are spectrally generic: their $\al$ values do not
differ significantly from randomly labeled text.  They represent a
baseline spectral signature---generic academic prose---against which
other categories differentiate.  Despite the failure to reject
individually, their rank positions are stable across all tests
(word shuffle, LOOCV, cross-model), and q-bio.MN achieves 100\%
LOOCV accuracy from its extreme rank position alone
(Section~\ref{sec:loocv}).  Rank ordering is a more robust signal
than per-category significance.
...
The eigenvalue tails are heavy-tailed: power-law is not ruled out
for 24/25 categories (CSN $p \geq 0.10$; math.LO rejected at
$p = 0.032$).  However, a Vuong log-likelihood ratio test
\citep{clauset2009} finds lognormal weakly preferred ($R < 0$) for
all 25 categories, though the preference is not significant for
most (21/25 have $p > 0.30$).  The two distributions are
statistically indistinguishable at this sample size.

The tail exponent $\al$ is used throughout as a log-log tail
slope---a shape parameter---not as evidence for a specific
generative mechanism.  $\al$ discriminates domains regardless of
whether the underlying distribution is power-law, lognormal, or
some other heavy-tailed family.
...
\item The greek\_literary layer-5 anomaly is consistent with
tokenization (82.8\% single-character tokens for polytonic Greek
vs.\ 12.8\% for English).  This is not merely a confound but a
diagnostic: early-layer compression operates preferentially on
well-represented subword units, and the anomaly reveals
vocabulary-dependent spectral processing.  A definitive isolation
would require a Greek-specific tokenizer (e.g., GreekBERT) on the
same texts, which is not possible without swapping models.
