...
of documentary Koine ($\pm 0.01$), robust to subsampling
(CV $= 0.2\%$), and plausible as a power-law fit (24/25 categories
pass CSN bootstrap, though lognormal fits equally well).
...
via Clauset--Shalizi--Newman bootstrap (24/25 categories are not
rejected as power-law), three independent estimators (OLS, MLE,
Hill), and matched null controls
...
texts, remove it from its category, recompute $\al$ for that
category using the remaining 399 texts, and assign the held-out
text to the category whose leave-one-out $\al$ changed least
(i.e., the category $c$ minimizing
$|\al_c^{(-i)} - \al_c^{\text{full}}|$).  Intuitively, a text
belongs to the category whose spectral exponent it least disturbs.
All 25 categories' $\al$ values are recomputed with the held-out
text removed from its true category, and classification is by
nearest $\al$ among the 25 leave-one-out values.
...
($\pm 0.01$), robust to subsampling (CV $= 0.2\%$), and plausible
as a power-law fit (24/25 categories not rejected by CSN bootstrap,
though lognormal fits equally well).  LOOCV achieves 52.5\%
...
\url{https://github.com/red40mademedoit/spectral-universality}.
